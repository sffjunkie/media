...
sys.path.insert(0, p)
p = os.path.abspath(os.path.join(test_path, '..', '..', '..', 'mogul', 'src'))
sys.path.insert(0, p)
...
data_path = os.path.join(base_path, 'data', 'media', 'asf')
...
def read_ASF(filename):
    logger.debug('ASFHandler: Reading file %s' % filename)
    with open(filename, 'rb') as ds:
        doctype = ASFHandler.can_handle(ds)
...
def test_All_ASF():
    for filename in glob.glob(os.path.join(data_path, '*.wm?')):
        read_ASF(filename)
    
if __name__ == '__main__':
    test_All_ASF()
    #read_ASF(filename('test.wmv'))
