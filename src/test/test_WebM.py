...
data_path = os.path.join(base_path, 'data', 'media', 'webm')
...
from mogul.media.webm import WebMHandler

def filename(name):
    return os.path.join(data_path, name)

def read_WebM(filename):
    logger.debug('WebMHandler: Reading file %s' % filename)
    with open(filename, 'rb') as ds:
        doctype = WebMHandler.can_handle(ds)
...
def test_All_WebM():
    for filename in glob.glob(os.path.join(data_path, '*.webm')):
        read_WebM(filename)
    
if __name__ == '__main__':
    test_All_WebM()
    #read_WebM(filename('test.webm'))
